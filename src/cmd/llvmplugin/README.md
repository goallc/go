...
| Fixed-width pointer-vector SSA values | Supported | An SSA vector remains one `gc-live` operand and one same-typed `gc.relocate`; it is not split into lanes. A fixed pointer-vector alloca uses one locals-pointer-map bit per lane. |
